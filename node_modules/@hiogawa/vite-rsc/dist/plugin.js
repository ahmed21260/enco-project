	...
	return [
		...
		},
		{
			...
			generateBundle(_options, bundle) {
				...
				if (this.environment.name === "client") {
					const filterAssets = rscPluginOptions.copyServerAssetsToClient ?? ((id) => id.endsWith(".css"));
					for (const asset of Object.values(rscBundle)) if (asset.type === "asset" && filterAssets(asset.fileName)) this.emitFile({
						type: "asset",
	...
	return [{
		...
		load(id) {
			if (id === "\0virtual:vite-rsc/encryption-key") {
				if (this.environment.mode === "build") return `export default () => __vite_rsc_define_encryption_key`;
		...
		renderChunk(code, chunk) {
			if (code.includes("__vite_rsc_define_encryption_key")) {
				...
				const normalizedPath = normalizeRelativePath(path.relative(path.join(chunk.fileName, ".."), "__vite_rsc_encryption_key.js"));
				const replacement = `import(${JSON.stringify(normalizedPath)}).then(__m => __m.default)`;
				code = code.replaceAll("__vite_rsc_define_encryption_key", () => replacement);
				return { code };
			}
		},
		generateBundle() {
			if (this.environment.name === "rsc" && emitEncryptionKey) this.emitFile({
				type: "asset",
