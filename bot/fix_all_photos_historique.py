#!/usr/bin/env python3
"""
Script de correction et de synchronisation complète des photos historiques ENCO
...
def get_telegram_id_from_photo_file_id(photo_file_id):
    # Gère les formats bruts ou Firebase Storage
    if not photo_file_id:
        return None
    if photo_file_id.endswith('.jpg'):
        # Format Firebase Storage : PREFIX_TYPE_TELEGRAMID.jpg
...
def find_photo_url_in_photos_collection(telegram_id, photos_docs):
    for doc in photos_docs:
        data = doc.to_dict()
        # On matche sur l'ID Telegram extrait
        pfid = data.get('photo_file_id')
        if pfid and get_telegram_id_from_photo_file_id(pfid) == telegram_id:
            return data.get('photoURL') or data.get('url')
    return None

def find_local_photo_path(telegram_id):
    # Cherche un fichier local qui contient l'ID Telegram
...
def upload_and_create_photo_doc(local_path, telegram_id, db):
    # Upload la photo et crée le doc Firestore manquant
...
def main(dry_run=True):
    print(f"=== Correction historique des photos ENCO (dry_run={dry_run}) ===")
    if not db:
    ...
    rapport = {'corrections': [], 'manquantes': [], 'creations': [], 'timestamp': datetime.now().isoformat()}
    for col in PHOTO_COLLECTIONS:
        ...
        for doc in docs:
            ...
            for field in col['fields']:
                # Peut être une liste (photos_file_ids) ou un champ unique
                value = data.get(field)
                if isinstance(value, list):
                    new_urls = []
                    for pfid in value:
                        telegram_id = get_telegram_id_from_photo_file_id(pfid)
                        url = find_photo_url_in_photos_collection(telegram_id, photos_docs)
                        if not url:
                            local_path = find_local_photo_path(telegram_id)
                            if local_path:
                                if not dry_run:
                                    url = upload_and_create_photo_doc(local_path, telegram_id, db)
                                    # Refresh photos_docs
                                    photos_docs = list(photos_ref.stream())
                                rapport['creations'].append({'telegram_id': telegram_id, 'local_path': local_path, 'doc': doc_id, 'collection': col_name})
                        if url:
                            new_urls.append(url)
                        else:
    ...
    # Rapport final
    with open('fix_all_photos_historique_report.json', 'w', encoding='utf-8') as f:
        json.dump(rapport, f, indent=2, ensure_ascii=False)
    print(f"\n=== Rapport généré : fix_all_photos_historique_report.json ===")
    print(f"Corrections : {len(rapport['corrections'])}")
    print(f"Photos créées : {len(rapport['creations'])}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Correction historique des photos ENCO")
